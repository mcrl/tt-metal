...
def test_local_reduce_moe_output_basic(device, num_local_experts, num_tokens, hidden_dim):
    ...
    # Token 0: expert 0, pos 0, value 10.0, weight 0.5 -> 5.0
    expected_0 = 10.0 * 0.5
    # Token 1: expert 1, pos 0, value 20.0, weight 0.3 -> 6.0
    expected_1 = 20.0 * 0.3

    logger.info(f"  Expected token 0 value: {expected_0}")
    logger.info(f"  Actual token 0 value: {output_torch[0, 0].item()}")
    logger.info(f"  Expected token 1 value: {expected_1}")
    logger.info(f"  Actual token 1 value: {output_torch[1, 0].item()}")
    ...
    for t in range(min(8, num_tokens)):
        logger.info(f"    Token {t}: {output_torch[t, :4].tolist()}")

    logger.info(f"  TTNN output (first 8 tokens, first 4 dims):")
    for t in range(min(8, num_tokens)):
        logger.info(f"    Token {t}: {output_tt_torch[t, :4].tolist()}")

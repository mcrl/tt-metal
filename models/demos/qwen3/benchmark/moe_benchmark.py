...
class Qwen3MoeSparseMoeBlock(nn.Module):
    ...
    def __init__(self, layer_idx: int, mesh_device: ttnn.Device, ccl,
        ...
        self.experts = nn.ModuleList(
            [
                Qwen3MoeMLP(intermediate_size=moe_intermediate_size, mesh_device=mesh_device, hidden_size=hidden_size)
    ...
    def setup_tt(self, mode, mem_cfg, bs = None):
        ...
        self.bs = bs
        if bs is not None:
            self.selected_experts_rr = ttnn.from_torch(
                (torch.arange(bs * self.top_k) % self.num_experts).reshape(bs, self.top_k),
                device=self.mesh_device,
                mesh_mapper=ttnn.ReplicateTensorToMesh(self.mesh_device),
                dtype=ttnn.uint16,
    ...
    def forward_tt(self, hidden_states: ttnn.Tensor, mem_cfg, mode) -> ttnn.Tensor:
    ...
    @profile_trace("Qwen3MoeSparseMoeBlock", level=3)
    def forward(self, hidden_states: ttnn.Tensor, mem_cfg, mode) -> ttnn.Tensor:
        if mode == InferenceMode.PREFILL:
            batch_size, sequence_length, hidden_dim = hidden_states.shape
        elif mode == InferenceMode.DECODE:
            _, sequence_length, batch_size, hidden_dim = hidden_states.shape

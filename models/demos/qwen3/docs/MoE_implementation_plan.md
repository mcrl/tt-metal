...
- **B**: Batch size
- **S**: Sequence length
- **T**: Number of input tokens. T = B * S
...
- Contains global expert indices owned by this device
- Device 0: [0, 1, 2, ..., 15]
- Device 1: [16, 17, 18, ..., 31]
- ...
- Device 7: [112, 113, 114, ..., 127]
- Used to determine which experts to process on each device
...
- Computation:
  - For each expert e on this device (E/D experts total)
  - Take T_e tokens routed to expert e
...
- Computation:
  - For each expert e on this device (E/D experts total)
  - Take T_e combined activations for expert e
  - Perform: (T_e x H') @ (H' x H) = T_e x H

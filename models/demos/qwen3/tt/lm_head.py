...
from models.demos.qwen3.utils.composite_ops import mesh_scatter
from models.demos.qwen3.utils.config_dataclass import FromWeightConfig, LinearConfig, MeshDeviceStub, OpConfigBase
...
class LMHead:
    """TT implementation of Language model head for Qwen3."""
    ...
    @classmethod
    def _get_model_dims_from_cfg(cls, hf_config) -> tuple[int, int]:
        """Get the dimensions of the model from the config.

        Args:
            hf_config: Model configuration object.

        Returns:
            Tuple containing the hidden dimension and vocab_size of the LMHead.
        """
    ...
    def forward_prefill(cls, x: ttnn.Tensor, cfg: RunPrefillConfig) -> ttnn.Tensor:
        ...
        # Extract fields from ProgramConfigData manually
        pc_gen = cfg["linear_pc_gen"]
        output = ttnn.linear(
            x,
            program_config=cls._get_prefill_pc(
                seq_len=seq_len,

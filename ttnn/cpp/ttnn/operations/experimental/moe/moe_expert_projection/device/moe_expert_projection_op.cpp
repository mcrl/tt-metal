...
#include "moe_expert_projection_op.hpp"
...
void MoeExpertProjection::validate(const std::vector<Tensor>& input_tensors) const {
...
std::vector<TensorSpec> MoeExpertProjection::compute_output_specs(
...
std::vector<Tensor> MoeExpertProjection::create_output_tensors(
...
tt::tt_metal::operation::ProgramWithCallbacks MoeExpertProjection::create_program(
    ...
    return moe_expert_projection_single_core(
        hidden_states,
        routed_tokens,
        num_routed_tokens,
        expert_weights,

...
tt::tt_metal::operation::ProgramWithCallbacks local_reduce_moe_output(
    ...
    constexpr uint32_t INPUT_CB_NUM_ROWS = 2;
    ...
    // CB 4: Weight scalar buffer
    constexpr uint32_t cb_id_weight_scalar = tt::CBIndex::c_4;
    constexpr uint32_t tile_size = 32 * 32 * sizeof(uint16_t);
    ...
    // CB 5: Accumulation buffer
    constexpr uint32_t cb_id_accum = tt::CBIndex::c_5;
    const uint32_t num_tiles_per_row = (hidden_dim + 1023) / 1024;
    ...
    // CB 7: Temporary buffer for tile-to-row-major conversion

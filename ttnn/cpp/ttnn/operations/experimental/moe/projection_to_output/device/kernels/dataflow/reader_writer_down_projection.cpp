...
void kernel_main() {
    ...
    const uint32_t token_idx_map_addr = get_arg_val<uint32_t>(1);
    const uint32_t routed_tokens_addr = get_arg_val<uint32_t>(2);
    ...
    const uint32_t down_proj_weights_addr = get_arg_val<uint32_t>(5);
    const uint32_t output_addr = get_arg_val<uint32_t>(6);
    const uint32_t num_tokens = get_arg_val<uint32_t>(7);
    const uint32_t hidden_dim = get_arg_val<uint32_t>(8);
    const uint32_t expert_dim = get_arg_val<uint32_t>(9);
    const uint32_t experts_per_device = get_arg_val<uint32_t>(10);
    const uint32_t max_tokens_per_expert = get_arg_val<uint32_t>(11);
    const uint32_t top_k = get_arg_val<uint32_t>(12);
    const uint32_t num_experts_padded = get_arg_val<uint32_t>(13);

    // Circular buffers
    constexpr uint32_t cb_combined_row = tt::CBIndex::c_0;
    constexpr uint32_t cb_token_idx_map_row = tt::CBIndex::c_1;
    constexpr uint32_t cb_routed_row = tt::CBIndex::c_2;
    constexpr uint32_t cb_num_routed_row = tt::CBIndex::c_3;
    constexpr uint32_t cb_routing_weights_row = tt::CBIndex::c_4;
    constexpr uint32_t cb_weights_row = tt::CBIndex::c_5;
    ...
    constexpr auto token_idx_map_accessor_args = TensorAccessorArgs<combined_accessor_args.next_compile_time_args_offset()>();
    constexpr auto routed_accessor_args = TensorAccessorArgs<token_idx_map_accessor_args.next_compile_time_args_offset()>();
    ...
    const auto token_idx_map_accessor = TensorAccessor(token_idx_map_accessor_args, token_idx_map_addr, max_tokens_per_expert * sizeof(uint32_t));
    ...
    cb_reserve_back(cb_token_idx_map_row, 1);
    ...
    uint32_t l1_token_idx_map_addr = get_write_ptr(cb_token_idx_map_row);
    ...
    for (uint32_t e = 0; e < experts_per_device; e++){
        for (uint32_t tok = 0; tok < num_tokens; tok++) {
            ...
            uint64_t output_noc_addr = get_noc_addr(e * num_tokens + tok, output_accessor);
    ...
    for (uint32_t local_expert_idx = 0; local_expert_idx < experts_per_device; local_expert_idx++) {
        ...
        // Read routed tokens (using LOCAL index)
        ...
        // Read token index map and routing weights for this expert (using LOCAL index)
        uint64_t token_idx_map_noc_addr = get_noc_addr(local_expert_idx, token_idx_map_accessor);
        noc_async_read(token_idx_map_noc_addr, l1_token_idx_map_addr, max_tokens_per_expert * sizeof(uint32_t));
        ...
        volatile uint32_t* token_indices = reinterpret_cast<volatile uint32_t*>(l1_token_idx_map_addr);
        ...
        for (uint32_t token_idx = 0; token_idx < token_count; token_idx++) {
            ...
            DPRINT << "MOE: output_token_idx=" << output_token_idx << " token_idx= " << token_idx << ENDL();
    ...
    cb_push_back(cb_token_idx_map_row, 1);

...
void ProjectionToOutput::validate(const std::vector<Tensor>& input_tensors) const {
    ...
    const auto& token_idx_map = input_tensors[1];
    const auto& routed_tokens = input_tensors[2];
    const auto& num_routed_tokens = input_tensors[3];
    const auto& routed_token_weights = input_tensors[4];
    const auto& down_proj_weights = input_tensors[5];
    ...
    TT_FATAL(token_idx_map.storage_type() == StorageType::DEVICE, "token_idx_map must be on device");
    ...
    TT_FATAL(token_idx_map.layout() == Layout::ROW_MAJOR, "token_idx_map must be ROW_MAJOR");
    ...
    TT_FATAL(combined_shape.rank() == 3, "combined_activations must be 3D");
    ...
    TT_FATAL(combined_shape[2] == weights_logical_shape[1],
        "combined_activations width ({}) must match down_proj_weights input dim ({})",
        combined_shape[2], weights_logical_shape[1]);
...
std::vector<TensorSpec> ProjectionToOutput::compute_output_specs(const std::vector<Tensor>& input_tensors) const {
    ...
    const auto& combined_activations_shape = combined_activations.padded_shape();
    const auto& weights_logical_shape = down_proj_weights.logical_shape();

    const uint32_t num_local_experts = combined_activations_shape[0];
...
tt::tt_metal::operation::ProgramWithCallbacks ProjectionToOutput::create_program(
    ...
    const uint32_t expert_dim = combined_shape[2];
    const uint32_t hidden_dim = weights_logical_shape[2];
    ...
    return projection_to_output_single_core(
        ...
        input_tensors[1],  // token_idx_map
        input_tensors[2],  // routed_tokens
        input_tensors[3],  // num_routed_tokens
        input_tensors[4],  // routed_token_weights
        input_tensors[5],  // down_proj_weights
        output_tensors[0],
        num_tokens,
        hidden_dim,
        expert_dim,

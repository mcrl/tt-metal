...
#include "moe_down_projection_op.hpp"
#include "moe_down_projection_program_factory.hpp"
#include "ttnn/tensor/tensor.hpp"

namespace ttnn::operations::experimental::moe {

void MoeDownProjection::validate(const std::vector<Tensor>& input_tensors) const {
    TT_FATAL(input_tensors.size() == 6, "MoeDownProjection requires 6 input tensors");
...
std::vector<TensorSpec> MoeDownProjection::compute_output_specs(const std::vector<Tensor>& input_tensors) const {
    const auto& combined_activations = input_tensors[0];
...
std::vector<Tensor> MoeDownProjection::create_output_tensors(
...
tt::tt_metal::operation::ProgramWithCallbacks MoeDownProjection::create_program(
    const std::vector<Tensor>& input_tensors,
    std::vector<Tensor>& output_tensors) const {

    return moe_down_projection_multi_core(
